--[[
	...
	StarRayfield Interface Suite
	Custom fork for StarScript
...
local RayfieldLibrary = {
	...
	Theme = {
		...
		StarForge = {
			...
			Topbar = Color3.fromRGB(14, 18, 32),
			Shadow = Color3.fromRGB(5, 8, 18),

			NotificationBackground = Color3.fromRGB(14, 18, 31),
			NotificationActionsBackground = Color3.fromRGB(226, 233, 255),

			TabBackground = Color3.fromRGB(18, 24, 40),
			TabStroke = Color3.fromRGB(46, 63, 102),
			TabBackgroundSelected = Color3.fromRGB(72, 118, 255),
			TabTextColor = Color3.fromRGB(191, 206, 247),
			SelectedTabTextColor = Color3.fromRGB(248, 250, 255),

			ElementBackground = Color3.fromRGB(16, 22, 36),
			ElementBackgroundHover = Color3.fromRGB(23, 31, 49),
			SecondaryElementBackground = Color3.fromRGB(13, 18, 31),
			ElementStroke = Color3.fromRGB(44, 62, 97),
			SecondaryElementStroke = Color3.fromRGB(33, 48, 77),

			SliderBackground = Color3.fromRGB(55, 109, 235),
			SliderProgress = Color3.fromRGB(102, 151, 255),
			SliderStroke = Color3.fromRGB(141, 183, 255),

			ToggleBackground = Color3.fromRGB(16, 24, 38),
			ToggleEnabled = Color3.fromRGB(82, 135, 255),
			ToggleDisabled = Color3.fromRGB(74, 86, 115),
			ToggleEnabledStroke = Color3.fromRGB(152, 193, 255),
			ToggleDisabledStroke = Color3.fromRGB(112, 125, 156),
			ToggleEnabledOuterStroke = Color3.fromRGB(70, 111, 214),
			ToggleDisabledOuterStroke = Color3.fromRGB(57, 68, 96),

			DropdownSelected = Color3.fromRGB(26, 34, 54),
			DropdownUnselected = Color3.fromRGB(14, 20, 33),

			InputBackground = Color3.fromRGB(13, 20, 34),
			InputStroke = Color3.fromRGB(48, 66, 105),
			PlaceholderColor = Color3.fromRGB(145, 160, 198)
		},
...
local SelectedTheme = RayfieldLibrary.Theme.StarForge
local applyStarChrome
local styleElementFrame
local styleTabButton
local styleNotificationFrame

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = RayfieldLibrary.Theme[Theme] or RayfieldLibrary.Theme.StarForge or RayfieldLibrary.Theme.Default
	...
	for _, TabPage in ipairs(Elements:GetChildren()) do
		for _, Element in ipairs(TabPage:GetChildren()) do
			if Element.ClassName == "Frame" and Element.Name ~= "Placeholder" and Element.Name ~= "SectionSpacing" and Element.Name ~= "Divider" and Element.Name ~= "SectionTitle" and Element.Name ~= "SearchTitle-fsefsefesfsefesfesfThanks" then
				...
				if styleElementFrame then
					styleElementFrame(Element, Element.Name == "Paragraph" or Element.Name == "Label")
				end
			end
		end
	end
	...
	for _, TabButton in ipairs(TabList:GetChildren()) do
		if TabButton.ClassName == "Frame" and TabButton.Name ~= "Template" and TabButton.Name ~= "Placeholder" then
			local isSelected = Elements.UIPageLayout.CurrentPage and Elements.UIPageLayout.CurrentPage.Name == TabButton.Name
			if styleTabButton then
				styleTabButton(TabButton, isSelected)
			end
		end
	end

	for _, NotificationFrame in ipairs(Notifications:GetChildren()) do
		if NotificationFrame.ClassName == "Frame" and NotificationFrame.Name ~= "Template" then
			NotificationFrame.BackgroundColor3 = SelectedTheme.NotificationBackground
			if NotificationFrame:FindFirstChild("UIStroke") then
				NotificationFrame.UIStroke.Color = SelectedTheme.SliderProgress
			end
			if styleNotificationFrame then
				styleNotificationFrame(NotificationFrame)
			end
		end
	end

	if applyStarChrome then
		applyStarChrome()
	end
end
...
local function ensureNamedChild(parent, className, childName)
...
local function ensureCorner(frame, radius)
	local corner = frame:FindFirstChildOfClass("UICorner") or ensureNamedChild(frame, "UICorner", "StarCorner")
	corner.CornerRadius = radius
	return corner
end

local function setGradient(target, gradientName, rotation, colorA, colorB, transparencyA, transparencyB)
	local gradient = ensureNamedChild(target, "UIGradient", gradientName)
...
local function setAccent(parent, accentName, size, position, anchorPoint, transparency)
	local accent = ensureNamedChild(parent, "Frame", accentName)
	accent.AnchorPoint = anchorPoint or Vector2.new(0, 0)
	accent.Position = position
	accent.Size = size
	accent.BorderSizePixel = 0
	accent.BackgroundColor3 = SelectedTheme.SliderProgress
	accent.BackgroundTransparency = transparency or 0
	accent.ZIndex = parent.ZIndex + 1
	ensureCorner(accent, UDim.new(1, 0))
	setGradient(accent, "StarAccentGradient", 0, SelectedTheme.SliderProgress, SelectedTheme.SliderStroke, 0, 0)
	return accent
end

styleElementFrame = function(frame, useSecondary, customBackground, customStroke)
	if not frame or not frame:IsA("Frame") then
		return
	end

	local baseColor = customBackground or (useSecondary and SelectedTheme.SecondaryElementBackground or SelectedTheme.ElementBackground)
	local strokeColor = customStroke or (useSecondary and SelectedTheme.SecondaryElementStroke or SelectedTheme.ElementStroke)

	frame.BackgroundColor3 = baseColor
	frame.ClipsDescendants = true

	if frame:FindFirstChild("UIStroke") then
		frame.UIStroke.Color = strokeColor
		frame.UIStroke.Thickness = 1.15
	end

	ensureCorner(frame, UDim.new(0, 10))
	setGradient(
		frame,
		"StarPanelGradient",
		90,
		baseColor:Lerp(SelectedTheme.TextColor, 0.05),
		baseColor:Lerp(SelectedTheme.Shadow, 0.18),
		0.03,
		0.2
	)
	setAccent(frame, "StarAccent", UDim2.new(0, 3, 1, -12), UDim2.new(0, 6, 0, 6), Vector2.new(0, 0), 0.1)
end

styleTabButton = function(tabButton, isSelected)
	if not tabButton or not tabButton:IsA("Frame") then
		return
	end

	local baseColor = isSelected and SelectedTheme.TabBackgroundSelected or SelectedTheme.TabBackground
	local edgeTransparency = isSelected and 0.08 or 0.45

	tabButton.ClipsDescendants = true
	tabButton.Size = UDim2.new(tabButton.Size.X.Scale, tabButton.Size.X.Offset, 0, 34)
	tabButton.BackgroundColor3 = baseColor

	if tabButton:FindFirstChild("UIStroke") then
		tabButton.UIStroke.Color = isSelected and SelectedTheme.SliderStroke or SelectedTheme.TabStroke
		tabButton.UIStroke.Thickness = isSelected and 1.25 or 1.05
	end

	ensureCorner(tabButton, UDim.new(0, 10))
	setGradient(
		tabButton,
		"StarTabGradient",
		0,
		baseColor:Lerp(SelectedTheme.TextColor, isSelected and 0.12 or 0.05),
		baseColor:Lerp(SelectedTheme.Shadow, 0.24),
		isSelected and 0.02 or 0.08,
		isSelected and 0.18 or 0.26
	)
	setAccent(tabButton, "StarTabAccent", UDim2.new(0, 3, 1, -10), UDim2.new(0, 5, 0, 5), Vector2.new(0, 0), edgeTransparency)
end

styleNotificationFrame = function(notificationFrame)
	if not notificationFrame or not notificationFrame:IsA("Frame") then
		return
	end
	...
	if notificationFrame:FindFirstChild("UIStroke") then
		notificationFrame.UIStroke.Color = SelectedTheme.SliderProgress
		notificationFrame.UIStroke.Thickness = 1.1
	end

	ensureCorner(notificationFrame, UDim.new(0, 12))
	setGradient(
		notificationFrame,
		"StarNotificationGradient",
		90,
		SelectedTheme.NotificationBackground:Lerp(SelectedTheme.TextColor, 0.05),
		SelectedTheme.NotificationBackground:Lerp(SelectedTheme.Shadow, 0.2),
		0.02,
		0.18
	)
	setAccent(notificationFrame, "StarNotificationAccent", UDim2.new(0, 4, 1, -14), UDim2.new(0, 7, 0, 7), Vector2.new(0, 0), 0.05)
end

applyStarChrome = function()
	...
	if Main:FindFirstChild("UIStroke") then
		Main.UIStroke.Color = SelectedTheme.ElementStroke:Lerp(SelectedTheme.TextColor, 0.08)
		Main.UIStroke.Thickness = 1.2
	end
	...
	setGradient(
		...
		"StarWindowGradient",
		90,
		SelectedTheme.Background:Lerp(SelectedTheme.TextColor, 0.03),
		SelectedTheme.Background:Lerp(SelectedTheme.Shadow, 0.18),
		0.03,
		0.12
	)
	setGradient(
		...
		"StarTopbarGradient",
		0,
		SelectedTheme.Topbar:Lerp(SelectedTheme.TextColor, 0.05),
		SelectedTheme.Topbar:Lerp(SelectedTheme.Shadow, 0.18),
		0,
		0.14
	)
	setAccent(Topbar, "StarTopbarAccent", UDim2.new(1, 0, 0, 2), UDim2.new(0, 0, 1, -2), Vector2.new(0, 0), 0)

	local backdrop = ensureNamedChild(Main, "Frame", "StarBackdrop")
	backdrop.AnchorPoint = Vector2.new(0, 0)
	backdrop.Position = UDim2.new(0, 12, 0, 48)
	backdrop.Size = UDim2.new(1, -24, 1, -60)
	backdrop.BorderSizePixel = 0
	backdrop.BackgroundColor3 = SelectedTheme.SecondaryElementBackground
	backdrop.BackgroundTransparency = 0.35
	backdrop.ZIndex = 0
	ensureCorner(backdrop, UDim.new(0, 12))
	setGradient(
		backdrop,
		"StarBackdropGradient",
		90,
		SelectedTheme.SecondaryElementBackground:Lerp(SelectedTheme.TextColor, 0.02),
		SelectedTheme.SecondaryElementBackground:Lerp(SelectedTheme.Shadow, 0.15),
		0.02,
		0.12
	)
	...
	if Main:FindFirstChild("Search") then
		...
		if Main.Search:FindFirstChild("UIStroke") then
			Main.Search.UIStroke.Color = SelectedTheme.SliderProgress
			Main.Search.UIStroke.Thickness = 1.1
		end
	end
end
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		newNotification.UIStroke.Color = SelectedTheme.SliderProgress
		newNotification.Icon.ImageColor3 = SelectedTheme.TextColor
		if styleNotificationFrame then
			styleNotificationFrame(newNotification)
		end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('StarRayfield Settings', 0, true)

	if TabList['StarRayfield Settings'] then
		TabList['StarRayfield Settings'].LayoutOrder = 1000
	end

	if Elements['StarRayfield Settings'] then
		Elements['StarRayfield Settings'].LayoutOrder = 1000
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	Topbar.Title.Text = Settings.Name or "StarRayfield"
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "StarRayfield"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Forged Interface Suite"

	if Settings.LoadingTitle ~= "Rayfield Interface Suite" then
		LoadingFrame.Version.Text = "StarRayfield UI"
	end
	...
	do
		local requestedTheme = Settings.Theme or 'StarForge'
		local success, result = pcall(ChangeTheme, requestedTheme)
		if not success then
			local starSuccess, starResult = pcall(ChangeTheme, 'StarForge')
			if not starSuccess then
				local defaultSuccess, defaultResult = pcall(ChangeTheme, 'Default')
			...
			warn('issue rendering theme. using fallback')
			print(result)
		end
	end
	...
	if not Settings.DisableRayfieldPrompts then
		task.spawn(function()
			while not rayfieldDestroyed do
				...
				RayfieldLibrary:Notify({
					Title = "StarRayfield",
					Content = "StarRayfield is running with the local custom fork.",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			styleElementFrame(Button, false)
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			styleElementFrame(ColorPicker, false)
		...
		function Tab:CreateLabel(LabelText : string, Icon: number, Color : Color3, IgnoreTheme : boolean)
			...
			styleElementFrame(Label, true, Label.BackgroundColor3, Label.UIStroke.Color)
		...
		function Tab:CreateParagraph(ParagraphSettings)
			...
			styleElementFrame(Paragraph, true)
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			styleElementFrame(Dropdown, false)
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			styleElementFrame(Keybind, false)
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			styleElementFrame(Toggle, false)
		...
		function Tab:CreateSlider(SliderSettings)
			...
			styleElementFrame(Slider, false)
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['StarRayfield Settings'])
		end)
	end)
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "StarRayfield Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "StarRayfield Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('StarRayfield Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "StarRayfield Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
